...
interface WhatsAppButtonProps {
  ...
  message?: string;
  /** Fase 3: cuando Alberto se marca "en línea" desde el panel admin, el
   * mismo botón cambia de texto/estilo para invitar a escribir ahora mismo. */
  isOnline?: boolean;
}

/** Botón flotante permanente — abajo a la derecha en desktop, accesible en móvil. */
export function WhatsAppButton({ whatsappNumber, message, isOnline = false }: WhatsAppButtonProps) {
  ...
  return (
    <a
      href={buildWhatsAppLink(
        ...
        message ??
          (isOnline
            ? "Hola Beto, vi que estás en línea. Me gustaría consultarte ahora."
            : "Hola Beto, estoy interesado en reservar una consulta."),
      )}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={isOnline ? "Beto está en línea — contáctalo ahora por WhatsApp" : "¿Necesitas ayuda? Escríbenos por WhatsApp"}
    ...
    >
      ...
      <span className="hidden font-mono text-xs uppercase tracking-[0.1em] sm:inline">
        {isOnline ? "En línea · Contactar ahora" : "¿Necesitas ayuda?"}
      </span>
    </a>
  );
}
